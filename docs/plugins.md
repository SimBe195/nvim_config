...
| `snacks.nvim` | Core UI/workflow plugin for dashboard, picker, bigfile handling, lazygit, notifications, rename support, statuscolumn, and more. It is the configured fuzzy picker/search surface; its explorer module is disabled because `mini.files` owns file management. | `<leader><space>`, `<leader>,`, `<leader>/`, `<leader>:`, `<leader>f*`, `<leader>s*`, `<leader>x*`, `<leader>uC`, `<leader>gg`, `<leader>gf`, `<leader>gl`, `<leader>p` |
...
| `mini.files` | Edit-as-buffer file manager for creating, deleting, renaming, moving, and synchronizing filesystem changes. It is the primary explorer. | `<leader>e`, `<leader>E`, `<leader>fe`; inside mini.files: `l`, `h`, `q`, `g.`, `gc`, `<C-s>`, `<C-w>s`, `<C-w>v`, `<C-w>S`, `<C-w>V` |
...
| `nvim-lspconfig` | Configures built-in LSP clients. LSP maps are attached buffer-locally when a server starts. | `K`, `gK`, `gd`, `gD`, `gi`, `gy`, `gr`, `<leader>la`, `<leader>lr`, `<leader>lR`, `<leader>li`, `<leader>lA`, clangd-only `<leader>lh` |
| `mason.nvim` | Installs external tools such as formatters, linters, LSP servers, and DAP adapters. | `<leader>lm` |
...
| `conform.nvim` | Formatting engine for format-on-save and manual formatting. Big files opt out of autoformat. | `<leader>lf`, `<leader>lF`; `:FormatDisable`, `:FormatEnable`, `:FormatToggle` |
...
| `inc-rename.nvim` | Incremental rename command used by the LSP map. | `<leader>lA` |
...
| `venv-selector.nvim` | Select and activate Python virtual environments. | `<leader>lv` in Python |
| `clangd_extensions.nvim` | Extra clangd features for C and C++. | clangd-only `<leader>lh` |
| `SchemaStore.nvim` | JSON schema catalog for `jsonls`. | None |
| `crates.nvim` | Cargo.toml crate completion, hover, and actions. | None |
| `rustaceanvim` | Rust LSP and DAP integration. | Rust buffers: `<leader>lC`, `<leader>dR` |
...
| `nvim-dap-python` | Python DAP adapter integration and test debugging helpers. | `<leader>dPt`, `<leader>dPc` in Python |
